...
def test():    # 1300 ~ 1600
    ...
    os.makedirs("Joint/results/phases", exist_ok=True)
    ...
    coeffs_file_path = os.path.join('Joint/results', 'coeffs_joint.txt')
    ...
    for idx, (real_img, sim_img, true_coeffs) in enumerate(test_loader):
        ...
        if idx % 60 == 0:
            ...
            sample_phase_dir = os.path.join('Joint/results/phases', f'sample_{idx}')
    ...
    if total_samples > 0:
        ...
        # 保存到文件
        rmse_file_path = os.path.join('Joint/results', 'rmse_per_mode.txt')
